...
-- Error Estimate
local error_protofield = ProtoField.none("stamp.error_estimate", "Error Estimate", base.HEX)
local error_s_protofield = ProtoField.bool("stamp.error_estimate.s", "S", 8,
	{ [1] = "Synchronized", [2] = "Not synchronized" }, 0x80,
	"Description")
local error_z_protofield = ProtoField.bool("stamp.error_estimate.z", "Z", 8,
	{ [1] = "Synchronized", [2] = "Not synchronized" }, 0x40,
	"Description")
local error_scale_protofield = ProtoField.uint8("stamp.error_estimate.scale", "Scale", base.UNIT_STRING, { "" }, 0x3f,
	"Scale")
local error_multiplier_protofield = ProtoField.uint8("stamp.error_estimate.multiplier", "Multiplier", base.DEC)
...
local sender_error_protofield = ProtoField.none("stamp.sender_error_estimate", "Error Estimate", base.HEX)
local sender_error_s_protofield = ProtoField.bool("stamp.sender_error_estimate.s", "S", 8,
	{ [1] = "Synchronized", [2] = "Not synchronized" }, 0x80,
	"Description")
local sender_error_z_protofield = ProtoField.bool("stamp.sender_error_estimate.z", "Z", 8,
	{ [1] = "Synchronized", [2] = "Not synchronized" }, 0x40,
	"Description")
local sender_error_scale_protofield = ProtoField.uint8("stamp.sender_error_estimate.scale", "Scale", base.UNIT_STRING,
	{ "" }, 0x3f, "Scale")
local sender_error_multiplier_protofield = ProtoField.uint8("stamp.sender_error_estimate.multiplier", "Multiplier",
	base.DEC)
...
-- TLV
local tlv_type_map = { [0xb3] = "DSCP ECN", [0x1] = "Padding"}
...
local function dissect_tlv(buffer, tree)
	...
	local tlv_length = buffer(2, 2):uint() + 4
	local tlv_type = buffer(1, 1):uint()
	local tlv_tree = tree:add(tlv_protofield, buffer(0, tlv_length))
...
local function dissect_reflector(buffer, authenticated, tree)
	...
	next_field_start = next_field_start + dissect_received_timestamp(buffer(next_field_start, timestamp_field_size), tree)
	if authenticated then
	...
	next_field_start = next_field_start + dissect_sender_timestamp(buffer(next_field_start, (timestamp_field_size + error_estimate_field_size)), tree)
...
function stamp_protocol.dissector(buffer, pinfo, tree)
	...
	if is_sender then
		...
		next_field_start = next_field_start + dissect_reflector(buffer(next_field_start), is_authenticated, reflector_body)
	end
